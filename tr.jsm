...
module.cmd_tr =
function cmd_tr(dest, msg, nick, ident, host, conn, relay) {
	var args = /^"((?:\\")*[^"]+(?:\\"[^"]*)*)" "((?:\\")*[^"]+(?:\\"[^"]*)*)" "((?:\\")*[^"]+(?:\\"[^"]*)*)"$/.exec(msg);
	args.shift();
...
module.cmd_rot13 =
function cmd_rot13(dest, msg, nick, ident, host, conn, relay) {
	conn.reply(dest, nick, tr(msg, this.ALPHABET, "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm"));
	return true;
}
module.cmd_rot47 =
function cmd_rot47(dest, msg, nick, ident, host, conn, relay) {
	conn.reply(dest, nick, tr(msg, "!\"#$%&\'()*+,-./0123456789:;<=>?@" + this.UPPER + "[\\]^_`" + this.LOWER + "{|}~", "PQRSTUVWXYZ[\\]^_`" + this.LOWER + "{|}~!\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNO"));
	return true;
}
module.cmd_revtr =
function cmd_revtr(dest, msg, nick, ident, host, conn, relay) {
	conn.reply(dest, nick, tr(msg, this.ALPHABET, this.REVUPPER + this.REVLOWER));
	return true;
}
module.cmd_rev =
function cmd_rev(dest, msg, nick, ident, host, conn, relay) {
	conn.reply(dest, nick, msg.reverse());
	return true;
}
module.cmd_encode =
function cmd_encode(dest, msg, nick, ident, host, conn, relay) {
	var args = msg.split(" "), msg = args.slice(1).join(" ");
	switch (args[0]) {
...
module.cmd_decode =
function cmd_decode(dest, msg, nick, ident, host, conn, relay) {
	var args = msg.split(" "), msg = args.slice(1).join(" ");
	switch (args[0]) {
	case "base64": case "b64":
 ...
 *
 * @param {string|Array.<string>} str The string to transform.
 * @param {string|Array.<string>} frm 
