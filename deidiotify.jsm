...
module.onMsg = function onMsg(e) {
	...
	case "ciao":
		if (this.prefs.ciao && msg.length == 1) {
			conn.notice(nick, "Welcome. Please note we do not support warez around here, as this is a free, open-source software network.");
			return true;
		}
		break;
	case "!list":
	...
	case "!kick": case "!k":
		...
		break;
	case "!ban": case "!b":
		if (this.prefs["!ban"] && (!msg[1] || msg[1] == nick) && aucgbot.okToKick(e)) {
			conn.send("MODE", dest, "+b", "*!*@" + host);
			return true;
		}
		break;
	case "!kick-ban": case "!kickban": case "!kb":
		if (this.prefs["!kb"] && (!msg[1] || msg[1] == nick) && aucgbot.okToKick(e)) {
			conn.send("KICK", dest, nick, ":you asked for it");
			conn.send("MODE", dest, "+b", "*!*@" + host);
			return true;
		}
