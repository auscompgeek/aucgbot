 ...
 *   Propeng, someemail@gmail.com, multi-server support
 *
...
{	var args = Array.prototype.slice.call(arguments);
	for (arg in args)
	{	var server = args[arg];
		this.netstart(server[0], server[1], server[2], server[3], server[4], server[5]);
	}
}
calcbot.netstart =
function startNetwork(serv, port, user, pass, nick, chans)
{	var channels = "bots";
...
{	switch (cmd)
	{	case "self-destruct": // Hehe, I had to put this in :D
		...
		case "connect":
			...
			argary.shift();
			this.netstart(argary[0], argary[1], "", argary[3], "", argary[2]);
			break;
		case "join":
